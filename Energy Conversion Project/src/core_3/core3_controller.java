...
public class core3_controller implements Initializable {
	...
	@FXML TextField current_i_1;
	@FXML CheckBox current_check_2;
	@FXML TextField current_i_2;
	...
	@FXML TextField num_turn_N1;
	@FXML TextField num_turn_N2;
	...
	public void initialize(URL arg0, ResourceBundle arg1) {
		fig_1.setImage(new Image("/figures/figure 3.png"));
	...
	private void buttonActions() {
		...
		calc_button.addEventFilter(MouseEvent.MOUSE_CLICKED, e -> {
			if(addData()) {
				...
				try { layout = Main.getCalculation_3_Layout();
	...
	private void check_Actions() {
		...
		current_check.addEventHandler(MouseEvent.MOUSE_CLICKED, e -> {
			if(current_check.isSelected()) {  	
				current_i_1.setDisable( false); 
			} else {  						
				current_i_1.setDisable( true );   
			}
		});
		
		current_check_2.addEventHandler(MouseEvent.MOUSE_CLICKED, E -> {
			if(current_check_2.isSelected()) {  	
				current_i_2.setDisable( false);
			} else {  						   
				current_i_2.setDisable( true );  
			}
		});
	...
	private boolean addData() {
		Main.newCoreData();
		__unit = unit_box.getValue();
		
		try {
			
			Main.getCore_data().insert(Float.parseFloat(permeability_field.getText()), "permeability");
			
			Main.getCore_data().insert(Float.parseFloat(num_turn_N1.getText()), "number of turns N1");
			Main.getCore_data().insert(Float.parseFloat(num_turn_N2.getText()), "number of turns N2");
			...
			if(current_check.isSelected()) {
				Main.getCore_data().insert(Float.parseFloat(current_i_1.getText()), "current i 1");
			}
			if(current_check_2.isSelected()) {
				Main.getCore_data().insert(Float.parseFloat(current_i_2.getText()), "current i 2");
			}
