 ...
 *     *******************************
 */	
public class cor3_calc_controller implements Initializable {
	...
	@FXML private Label num_turns_label_2;
	@FXML private Label current_label;
	@FXML private Label current_I2_label;
	...
	public void initialize(URL arg0, ResourceBundle arg1) {
		U1.setText(core3_controller.unit);
		U2.setText(core3_controller.unit);
		U3.setText(core3_controller.unit+"^2");
		U4.setText(core3_controller.unit+"^2");
		U5.setText(core3_controller.unit+"^2");
		U6.setText(core3_controller.unit+"^2");
		U7.setText(core3_controller.unit+"^2");
		solveReluctance();
	}
	...
	public void solveReluctance() {
		...
		float current_1 = Main.Main.getCore_data().getData("current i 1");
		float current_2 = Main.Main.getCore_data().getData("current i 2");
		
		float flux = Main.Main.getCore_data().getData("flux");
		
		float num_turn_N1 = Main.Main.getCore_data().getData("number of turns N1");
		float num_turn_N2 = Main.Main.getCore_data().getData("number of turns N2");
		...
		if(current_1 == 0) {
			current_1 = (((flux * total_Reluctance) - (num_turn_N2 * current_2)) / num_turn_N1);
		}
		
		if(current_2 == 0) {
			current_2 = (((flux * total_Reluctance) - (num_turn_N1 * current_1)) / num_turn_N2);
		}
		
		emf_tot = (num_turn_N1*current_1) + (num_turn_N2*current_2);
		calc_flux_dens(flux);
		
		if(flux == 0) {
			flux = emf_tot / total_Reluctance;
			
			//System.out.println("emf tot: "+emf_tot);
		...
		emf_label.setText(Float.toString(emf_tot));
		
		num_turns_label.setText(Float.toString(num_turn_N1));
		num_turns_label_2.setText(Float.toString(num_turn_N2));
		current_label.setText(Float.toString(current_1));
		current_I2_label.setText(Float.toString(current_2));
